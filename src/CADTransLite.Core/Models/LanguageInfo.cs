...
/// <summary>
/// Static catalog of all supported languages (6 primary + 5 extended = 11 total).
/// </summary>
public static class SupportedLanguages
{
    ...
    /// <summary>
    /// Languages supported by each translation engine, keyed by provider name.
    /// Engines not listed support every language in <see cref="All"/>.
    /// Codes are <see cref="LanguageInfo.Code"/> values (e.g., "ZH", "EN").
    /// </summary>
    /// <remarks>
    ...
    ///   (including Ukrainian).
    /// </remarks>
    private static readonly Dictionary<string, HashSet<string>> ProviderSupportedCodes = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    /// <summary>
    /// Returns the languages the given provider supports. Engines absent from
    /// <see cref="ProviderSupportedCodes"/> return the full <see cref="All"/> list.
    /// The returned <see cref="LanguageInfo"/> instances are the same references as in
    /// <see cref="All"/> (order preserved), so reference equality with current selections holds.
    /// </summary>
    public static IReadOnlyList<LanguageInfo> ForProvider(string? providerName)
    {
        if (!string.IsNullOrEmpty(providerName) &&
            ProviderSupportedCodes.TryGetValue(providerName, out var supported))
        {
            return All.Where(l => supported.Contains(l.Code)).ToList();
        }
        return All.ToList();
    }
    ...
    [
        ...
        new()
        {
            Code = "UK", DisplayName = "乌克兰语", IsPrimary = false,
            BaiduCode = "uk", TencentCode = "uk", MicrosoftCode = "uk",
        },
    ];
}
