...
public static class MTextRebuilder
{
    ...
    public static string ReplaceVisibleText(string originalSkeleton, string translatedText)
    {
        if (string.IsNullOrEmpty(originalSkeleton))
            return EscapeAndNormalize(translatedText ?? string.Empty);

        if (string.IsNullOrEmpty(translatedText))
            return originalSkeleton;

        // 转义特殊字符、规范化换行符（\r\n / \r → \P）并清除行内控制字符
        string escaped = EscapeAndNormalize(translatedText);
    ...
    /// <summary>
    /// 转义 MTEXT 特殊字符（\ { }），将各类换行符（CRLF / CR / LF）统一映射为段落符 \P，
    /// 并清除会破坏 DXF 行结构的其它控制字符（制表符 → 空格，其余 C0 控制符丢弃）。
    /// 顺序：先规范化换行、再转义、最后插入 \P，避免 \P 的反斜杠被重复转义。
    /// </summary>
    private static string EscapeAndNormalize(string text)
    {
        if (string.IsNullOrEmpty(text)) return text;
        string normalized = text.Replace("\r\n", "\n").Replace("\r", "\n");
        string escaped = EscapeMTextSpecial(normalized);
    ...
    /// <summary>
    /// 将 MTEXT 文本按 DXF 规范拆分为组码行块。
    /// DXF 要求单值行不超过约 256 字符：超过 250 字符的文本必须切成若干 252 字符的块，
    /// 前面若干块用组码 3，最后一个块用组码 1。
    /// 组码 3 / 1 的拼接是字面连接，因此切分点落在 \P、\\、\{ 等转义符中间也是安全的。
    /// </summary>
    /// <param name="mtextValue">已转义、已含 \P 的完整 MTEXT 文本。</param>
    /// <param name="maxChunk">每块最大字符数（默认 250，留余量在 256 限制内）。</param>
    /// <returns>形如 (组码, 文本) 的列表，末尾为组码 1。</returns>
    public static List<(string Code, string Text)> SplitMTextToDxfChunks(string mtextValue, int maxChunk = 250)
    {
        var chunks = new List<(string Code, string Text)>();
        if (string.IsNullOrEmpty(mtextValue))
        {
            chunks.Add(("1", string.Empty));
            return chunks;
        }

        int i = 0;
        while (i < mtextValue.Length)
        {
            int len = Math.Min(maxChunk, mtextValue.Length - i);
            string piece = mtextValue.Substring(i, len);
            i += len;
            // 仅最后一块用组码 1，其余用组码 3
            chunks.Add((i < mtextValue.Length ? "3" : "1", piece));
        }
        return chunks;
    }
