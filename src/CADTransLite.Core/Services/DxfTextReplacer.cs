...
public static class DxfTextReplacer
{
        ...
        List<string> log)
    {
        ...
        if (DxfRawParser.IsGroupCodeLine(lines[cell.TextLineNumber]))
        {
            log.Add($"[WARN] TABLE Handle={tableHandle} R{row}C{col} 跳过：目标行 {cell.TextLineNumber} 是组码行（'{lines[cell.TextLineNumber].Trim()}'），疑似映射错位，已跳过以避免损坏文件。");
            return false;
        }

        // Replace the value at the specific line number
        // TextLineNumber is the 0-based index of the value line in the lines array
        lines[cell.TextLineNumber] = DxfRawParser.SanitizeSingleLineText(newText);
        ...
        List<string> log)
    {
        ...
        // 写保护：目标行若是组码行，说明行号映射错位，跳过以免破坏整图结构
        if (DxfRawParser.IsGroupCodeLine(lines[ml.TextLineNumber]))
        {
            log.Add($"[WARN] MLEADER Handle={handle} 跳过：目标行 {ml.TextLineNumber} 是组码行（'{lines[ml.TextLineNumber].Trim()}'），疑似映射错位，已跳过以避免损坏文件。");
            return false;
        }

        // Replace the value at the specific line number
        lines[ml.TextLineNumber] = DxfRawParser.SanitizeSingleLineText(newText);
