...
public sealed class TranslationService
{
    ...
    /// <summary>Maximum number of texts per batch.
    /// For DeepLX (local free proxy) the provider already serializes and rate-limits per request,
    /// so a batch of 1 keeps the overall request rate low and avoids 429.</summary>
    private const int BatchSize = 1;

    /// <summary>Delay between batches in milliseconds (extra breathing room beyond the provider's own delay).</summary>
    private const int InterBatchDelayMs = 1000;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++)
        {
            ...
            if (translations.Count != batchCount)
            {
                // Mismatched batch result — this is an internal error, never silently fall back to
                // the original text (that would write source text into the target column and look
                // like a "translation"). Fail loudly instead.
                throw new InvalidOperationException(
                    $"翻译引擎返回的条数不匹配：期望 {batchCount} 条，实际 {translations.Count} 条。");
            }
            ...
            for (int j = 0; j < batchIndices.Count; j++)
            {
                ...
                // Do NOT silently fall back to the original text when the engine returns an empty
                // translation. That would write the source language into the target column and
                // masquerade a failed translation as a successful one (the user sees "翻译完成
                // 100/100" while the text is still the original). Keep the result empty so it is
                // skipped at write-back and surfaced as "untranslated" in the UI.
                items[itemIndex].TranslatedText = translated;
            }
