...
public static class DxfTextCleaner
{
    ...
    // -----------------------------------------------------------------
    // IsNonTextual — 无可翻译文字（纯数字 / 纯符号）
    // -----------------------------------------------------------------

    /// <summary>
    /// 判断文本是否“无可翻译文字”——即不含任何字母(A-Za-z)与中日韩(含假名/谚文)字符，
    /// 仅由数字、符号、标点、单位、空白等组成。此类内容（如 "12.5"、"±0.05"、"Ø50"、"1:100"、"—"）
    /// 通常无需翻译，应从 Excel 中剔除。
    /// 注意：含字母的内容（如 "R5"、"A-1"、"104x13.4x4"）视为有文字，予以保留。
    /// </summary>
    public static bool IsNonTextual(string text)
    {
        if (string.IsNullOrWhiteSpace(text))
            return true;
